...
import { UserPlus, Database, Search, FileUp, ClipboardList, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function CustomersPage() {
  ...
  const handleBulkImport = () => {
    ...
    try {
      ...
      toast({ title: "Import Successful", description: `Added ${count} customers.` });
      setImportData("");
  ...
  const seedData = () => {
    if (!firestore) return;
    toast({ title: "Seeding...", description: "Adding sample customers." });
    for (let i = 1; i <= 20; i++) {
      addDocumentNonBlocking(collection(firestore, 'customers'), {
        name: `Supplier ${i}`,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              ...
              <p className="text-muted-foreground">Directory of milk suppliers and account numbers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Dialog open={isImporting} onOpenChange={setIsImporting}>
                ...
                <DialogContent className="sm:max-w-[600px] rounded-3xl">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-black text-primary">Import Customers</DialogTitle>
                    <DialogDescription>
                      Paste your customer data in JSON format below. Follow the template structure.
                    </DialogDescription>
                  </DialogHeader>
            ...
            <Card className="mb-8 border-primary/20 bg-primary/5 rounded-3xl shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <CardContent>
                ...
                <div className="mt-6 flex justify-end">
                  <Button onClick={handleAddCustomer} className="rounded-full px-8">Save Customer</Button>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="rounded-3xl overflow-hidden border-none shadow-xl bg-card/50 backdrop-blur-sm">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow><TableCell colSpan={4} className="text-center py-20 text-muted-foreground animate-pulse">Loading supplier directory...</TableCell></TableRow>
                ) : filteredCustomers?.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-20">
                      <div className="flex flex-col items-center gap-2">
                        <ClipboardList className="w-10 h-10 text-muted-foreground/30" />
                        <p className="text-muted-foreground font-medium">No customers found matching your search.</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredCustomers?.map((customer) => (
                    <TableRow key={customer.id} className="group hover:bg-primary/5 transition-colors">
                      <TableCell className="font-black text-primary pl-6 text-lg">{customer.canNumber}</TableCell>
                      <TableCell className="font-semibold text-base">{customer.name}</TableCell>
                      <TableCell className="text-muted-foreground font-mono">{customer.accountNumber || "—"}</TableCell>
            ...
            <div className="p-4 bg-muted/20 text-center text-[10px] text-muted-foreground font-bold uppercase tracking-widest border-t">
              Total Registered Suppliers: {customers?.length || 0}
            </div>
          </Card>
        </div>
      </main>
