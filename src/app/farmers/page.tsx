...
import { UserPlus, Database, Search, FileUp, ClipboardList, CheckCircle2, AlertCircle, Download, FileSpreadsheet } from "lucide-react";
...
export default function FarmersPage() {
  ...
  const handleBulkImport = () => {
    ...
    try {
      ...
      if (trimmedData.startsWith('[')) {
        ...
        data = lines.slice(1).map(line => {
          ...
          headers.forEach((header, i) => {
            const key = header === 'can' ? 'canNumber' : 
                        header === 'account' ? 'accountNumber' : header;
            obj[key] = values[i];
          });
          return obj;
        });
      ...
      if (!Array.isArray(data)) throw new Error("Data format invalid. Please use the provided template.");
  ...
  const downloadExcelTemplate = () => {
    const headers = "name,canNumber,accountNumber";
    const example = "Rajesh Kumar,101,9876543210\nSuresh Singh,102,1234567890";
    ...
    toast({ title: "Template Downloaded", description: "Open this file in Excel to fill your farmer list." });
  };
              ...
              <Dialog open={isImporting} onOpenChange={setIsImporting}>
                ...
                <DialogContent className="sm:max-w-[600px] rounded-3xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Upload your list using our Excel-compatible CSV template.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="space-y-4 py-4">
                    <div className="flex items-center justify-between p-4 bg-primary/5 rounded-2xl border border-primary/10">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold">Step 1: Download Template</p>
                          <p className="text-xs text-muted-foreground">Get the Excel-ready CSV structure.</p>
                        </div>
                      </div>
                      <Button variant="outline" size="sm" onClick={downloadExcelTemplate} className="rounded-full h-8">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-primary flex items-center gap-2">
                        ...
                        Step 2: Paste Data (CSV or JSON)
                      </label>
                      <Textarea 
                        placeholder="Name,CAN,Account&#10;John Doe,001,9123456789&#10;Jane Smith,002,9123456788" 
                        value={importData}
